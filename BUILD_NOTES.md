...
- [x] Added exposure configuration UI (`app/templates/dashboard/partials/exposure_config.html`) to Exposures tab showing preset cards with exposure time, count, spacing, filter, binning, and total session duration. Info banner explains NEOCP astrometry approach (3–6 exposures, 60–120s each, 1–3 min spacing). Calibration note educates users that bias/darks/flats should be taken once per session in NINA and reused as master frames (not re-shot per target). Preset cards use `preset-grid`, `preset-card`, `preset-header`, and `preset-details` CSS classes. Endpoint `/dashboard/partials/exposure_config` loads presets via `list_presets()`.
- [x] Imaging configuration card now lets operators tweak the active preset (exposure time, count, spacing, binning, filter) inline and save overrides back to session state so automation runs with those custom values without redefining presets.
- [x] Exposures tab now includes a FITS preview workflow: recent captures are clickable and load a viewer panel (server converts the FITS to a z-scale PNG on the fly). A dedicated Solver tab mirrors the astrometry status panel so operators can monitor solves separately from the exposure controls.
