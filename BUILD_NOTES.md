...
- Inventory observing hardware (mount, OTA, camera, filters, focuser) and store normalized equipment profiles in the database; expose the same CRUD screens in the dashboard so users can add/edit profiles and mark one as active.
...
- Implement the `nina-bridge` service that authenticates to NINA's local API/WebSocket and exposes simplified REST endpoints to the rest of the system.
