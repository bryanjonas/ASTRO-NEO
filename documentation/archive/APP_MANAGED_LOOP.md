...
- `app/services/capture_loop.py` now implements the predict → slew → expose loop described above; it logs each exposure, records captures, and reports solver success/failure.
- `AutomationService.run_sequential_target_sequence()` now delegates to the capture loop for every target while keeping guiding start/stop and returning summary totals to the API.
- `documentation/QUICK_READ.md` reflects the app-managed loop and calls out the new `SESSION_STATE` exposure logging so the overview dashboard surfaces per-target progress.
