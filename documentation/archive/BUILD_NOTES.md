...
- [x] Pull ephemerides from MPC's API (fallback to static RA/Dec when offline) to compute per-minute positions; cache ephemerides per object-night in `neoephemeris` for reuse.
- [x] **JPL Horizons Integration** (`app/services/horizons_client.py`): Added authoritative topocentric ephemeris support for high-priority targets (score > 80 or recent observations < 24h). Horizons provides light-time correction, aberration, parallax, motion rates (RA/Dec arcsec/min), observing geometry (azimuth, elevation, airmass, solar/lunar elongations), predicted V magnitude, and 3σ positional uncertainty. System falls back to MPC ephemerides when Horizons unavailable. Database schema extended with motion rates, geometry fields, and source tracking (`NeoEphemeris.source`). Migration `0016_add_horizons_ephemeris_fields.py` (revision `f3a2b9c8d5e1`) added.
- [x] **Dynamic Target Scoring** (`app/services/target_scoring.py`): Implemented multi-factor composite scoring (0-100 scale) combining: MPC priority (30%), altitude/airmass (25%), time-to-set (15%), motion rate (10%), positional uncertainty (10%), and arc-extension value (10%). Each component scored independently with specific thresholds, then weighted for final priority. Scoring weights configurable via settings. Database schema extended with `composite_score` and `peak_altitude_deg` fields on `NeoObservability`. Migration `0017_add_composite_score_fields.py` (revision `a4b8c7d9e2f3`) applied.
...
- [x] **Fast-Mover Exposure Adaptation** (`app/services/presets.py`): Added motion-compensated exposure strategy for targets moving > 30 "/min. System calculates maximum exposure time to limit trailing < 5 pixels (using pixel scale from equipment profile, default 1.5"/pixel). For fast movers: reduces exposure duration, increases frame count to maintain total integration time (SNR), and reduces inter-exposure delay for tighter temporal sampling. Example: 60"/min motion reduces 120s exposure to 7.5s with 16× frames instead of 6×. Logging shows adaptation details for operator awareness.
- [x] **Two-Stage Target Acquisition** (`app/services/acquisition.py`): Implemented slew-confirm-verify workflow for accurate pointing: (1) Predict position using fresh Horizons ephemeris, (2) Slew to predicted coordinates, (3) Take 8s bin2 confirmation exposure, (4) Plate solve and calculate offset from prediction using haversine formula, (5) Refine pointing if offset > 120" threshold. Acquisition result includes predicted/solved coordinates, offset magnitude, and refinement status. Integrated into `capture_loop.py` as optional pre-imaging step (enabled by default for NEOCP targets).
- [x] **Synthetic Target Testing Support** (`app/services/synthetic_targets.py`): Enhanced synthetic target generation to support all new features: realistic motion rate distribution (50% slow, 30% moderate, 15% fast, 5% very fast), composite score calculation matching production algorithm, last observation timestamps (1-48h ago) for arc extension testing, all Horizons fields (motion rates, geometry, uncertainty), and logging showing motion rates per target. Targets remain at 30-45° altitude for immediate testability.
- [x] **Database Clear & Reseed** (`/dashboard/targets/clear`): Added destructive operation to wipe all NEOCP-related data (measurements, solutions, associations, submissions, observability, ephemerides, candidates) in correct foreign key order, then automatically reseed with fresh synthetic targets. Includes user confirmation dialog and orange-styled warning button. Preserves site config, equipment profiles, and weather data. Returns combined HTMX response updating both targets panel and status banner.
